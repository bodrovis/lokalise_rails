  ...
  module TaskDefinition
    class Exporter < Base
      ...
      def do_upload(f_path, r_path)
        with_exp_backoff(global_opts.max_retries_export) do
      ...
      def each_file
        ...
        loc_path = global_opts.locales_path
      ...
      def opts(full_p, relative_p)
        ...
        initial_opts = {
          ...
          filename: relative_p,
          lang_iso: global_opts.lang_iso_inferer.call(content)
        }

        initial_opts.merge global_opts.export_opts
      end
      ...
      def file_matches_criteria?(full_path)
        full_path.file? && proper_ext?(full_path) &&
          !global_opts.skip_file_export.call(full_path)
      end
    end
  end
end
