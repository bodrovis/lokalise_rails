...
module LokaliseRails
  module TaskDefinition
    class Importer < Base
      ...
      # @return [Hash]
      def download_files
        with_exp_backoff(global_opts.max_retries_import) do
          api_client.download_files project_id_with_branch, global_opts.import_opts
        end
      ...
      def process!(zip_entry)
        ...
        full_path = "#{global_opts.locales_path}/#{subdir}"
        FileUtils.mkdir_p full_path

        File.open(File.join(full_path, filename), 'w+:UTF-8') do |f|
          f.write global_opts.translations_converter.call(data)
        end
      rescue StandardError => e
      ...
      def proceed_when_safe_mode?
        return true unless global_opts.import_safe_mode && !Dir.empty?(global_opts.locales_path.to_s)

        $stdout.puts "The target directory #{global_opts.locales_path} is not empty!"
      ...
      def data_from(zip_entry)
        global_opts.translations_loader.call zip_entry.get_input_stream.read
      end
    end
  end
end
