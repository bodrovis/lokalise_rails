...
describe LokaliseRails::TaskDefinition::Importer do
  ...
  describe '.download_files' do
    it 'returns a proper download URL' do
      allow_project_id described_object, '672198945b7d72fc048021.15940510' do
        ...
        expect(described_object.global_opts.max_retries_import).to eq(5)
      end
    end
  ...
  describe '.import!' do
    it 'handles too many requests' do
      ...
      allow(described_object.global_opts).to receive(:max_retries_import).and_return(2)
      ...
      expect(described_object.global_opts).to have_received(:max_retries_import).exactly(1).times
    ...
    it 'halts when the API key is not set' do
      ...
      expect(-> { described_object.import! }).to raise_error(LokaliseRails::Error, /API token is not set/i)
    ...
    it 'halts when the project_id is not set' do
      allow_project_id described_object, nil do
        expect(-> { described_object.import! }).to raise_error(LokaliseRails::Error, /ID is not set/i)
        expect(count_translations).to eq(0)
      end
    end
  end
